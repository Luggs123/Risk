...
bool Fortification::fortificate2(Territory*take, int re) {
    ...
    for (int i = 0; i < num_nei; i++) {
        for (int j = 0; j < ptemp->get_number_controlled(); j++) {
            if ((take->get_neighbors()[i])->equals(ptemp->get_controlled()+j)) {
                cout << k << ". " << (ptemp->get_controlled() + j)->get_name() << " currently has " << (ptemp->get_controlled() + j)->get_troops() << " troops" << endl;
                neighbour.push_back((ptemp->get_controlled() + j));
                k++;
            }
